***schedule\*2019-12-12 20:24:45.0** ***face\*1222** ***comment\*0** ***favorite_border\*1**添加至喜欢删除

#### **BAT必考JVM系列专题**

![file](https://graph.baidu.com/resource/222abcf86c48dbddca06b01576150117.png)

#### **JVM内存结构**

![img](深入详解JVM内存模型与JVM参数详细配置  Spring For All.assets/1540886272779458d0b42c3)
...
其中**栈内存**可以再细分为**java虚拟机栈和本地方法栈,堆内存**可以划分为新生代和老年代,新生代中还可以再次划分为Eden区、From Survivor区和To Survivor区。

其中一部分是线程共享的，包括 Java 堆和方法区；另一部分是线程私有的，包括虚拟机栈和本地方法栈，以及程序计数器这一小部分内存。

**堆内存（Heap）**

java 堆（Java Heap）是Java 虚拟机所管理的内存中最大的一块。堆是被所有线程共享的区域，实在虚拟机启动时创建的。堆里面存放的都是**对象的实例**（new 出来的对象都存在堆中）。
...
![img](深入详解JVM内存模型与JVM参数详细配置  Spring For All.assets/15408862726358fb61a7caa)
...
栈帧又分为一下几个区域：**局部变量表、操作数栈、动态连接、方法出口**等。平时我们所说的变量存在栈中，这句话说的不太严谨，应该说局部变量存放在java虚拟机栈的局部变量表中。java的8中基本类型的局部变量的值存放在虚拟机栈的局部变量表中，如果是引用型的变量，则只存储对象的引用地址。

![file](https://graph.baidu.com/resource/222f84270b3ce980d8e0f01576153055.png)
...
![img](深入详解JVM内存模型与JVM参数详细配置  Spring For All.assets/1540887761666cd88840b4a)
