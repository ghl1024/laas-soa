...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/Jmeter 压力测试 TPS.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/Jmeter 压力测试 TPS.png)

**测试流程如下：**

首先下载 JMeter 安装包 可以去官网下载：[http://jmeter.apache.org](http://jmeter.apache.org/)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/1.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/1.png)

线程组属性内可以修改线程数、Ramp-Up 时间和循环次数。

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/2.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/2.png)

（2）在线程组上右键添加 HTTP 请求

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/3.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/3.png)

其属性包括 WEB 服务器的协议、服务器名称或 IP 和端口号，HTTP 请求的方法和路径。

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/4.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/4.png)

（3）在HTTP请求上右键添加一个监听器，可以根据自己的需求添加汇总报告、查看结果树等等。

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/5.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/5.png)

如下图所示为汇总报告，可以查看异常比例和吞吐量，方便调优。

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/6.png)](https://github.com/daydreamdev/MeetingFilm/raw/master/pic/seconds-kill/6.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/秒杀超卖.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/秒杀超卖.png)

悲观锁虽然可以解决超卖问题，但是加锁的时间可能会很长，会长时间的限制其他用户的访问，导致很多请求等待锁，卡死在这里，如果这种请求很多就会耗尽连接，系统出现异常。乐观锁默认不加锁，更失败就直接返回抢购失败，可以承受较高并发

[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/乐观锁扣库存.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/乐观锁扣库存.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/秒杀限流.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/秒杀限流.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/Redis 缓存库存信息.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/Redis 缓存库存信息.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/先更新数据库再更新缓存.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/先更新数据库再更新缓存.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/先更新数据库再删除缓存.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/先更新数据库再删除缓存.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/先更新数据库再更新缓存V2.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/先更新数据库再更新缓存V2.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/秒杀热点数据.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/秒杀热点数据.png)
...
[![img](daydreamdevseconds-kill 基于 Springboot + Redis + Kafka 的秒杀系统，乐观锁 + 缓存 + 限流 + 异步，TPS 从 500 优化到 3000.assets/消息队列缓冲.png)](https://github.com/gongfukangEE/gongfukangEE.github.io/raw/master/_pic/分布式/消息队列缓冲.png)
