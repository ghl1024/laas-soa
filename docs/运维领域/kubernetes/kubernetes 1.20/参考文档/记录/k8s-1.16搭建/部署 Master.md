...
- 签发证书需要用到 [生成 CA 证书](https://k8s.imroc.io/deploy/manual/bootstrapping-master/prepare.md#generate-ca-cert) 时创建的 CA 证书及其密钥文件，确保它们在当前目录
- 确保 cfssl 在当前环境已安装，安装方法参考 [这里](https://k8s.imroc.io/deploy/manual/bootstrapping-master/prepare.md#install-cfssl)
...
- 生成 kubeconfig 会用到之前[准备证书](https://k8s.imroc.io/deploy/manual/bootstrapping-master/#prepare-certs)时创建的证书与密钥，确保这些生成的文件在当前目录。
...
- 入口地址的域名或IP必须是在之前 [为 kube-apiserver 签发证书](https://k8s.imroc.io/deploy/manual/bootstrapping-master/#sign-certs-for-kube-apiserver) 的 hosts 列表里。
