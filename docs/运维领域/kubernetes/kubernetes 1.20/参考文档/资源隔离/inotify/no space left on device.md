...
系统调用 `inotify_add_watch` 失败，提示 `no space left on device`， 这是因为系统上进程 watch 文件目录的总数超出了最大限制，可以修改内核参数调高限制，详细请参考本书 [处理实践: inotify watch 耗尽](https://k8s.imroc.io/handle/runnig-out-of-inotify-watches/)
...
详细请参考本书 [案例分享: cgroup 泄露](https://k8s.imroc.io/summary/cgroup-leaking/)