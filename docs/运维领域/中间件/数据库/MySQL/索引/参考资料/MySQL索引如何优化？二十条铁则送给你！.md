...
![索引优化几大规则](MySQL索引如何优化？二十条铁则送给你！.assets/索引优化.png)
...
1. **补充**：有些地方说在`where`条件中使用`or`，索引会失效，造成全表扫描，这是个误区：
...
> 1. **存在非等号和等号混合判断条件时，在建立索引时，把等号条件的列前置**。如 `where a>? and b=?`，那么即使`a` 的区分度更高，也必须把 `b` 放在索引的最前列。
...
> 1. 假如`index(a,b,c)`, `where a=3 and b like 'abc%' and c=4`，`a`能用，`b`能用，`c`不能用。
