...
​	目前只要是实例设置为无锁表更, 那么所有的变更都是无锁的, 普通数据变更中也是无锁进行变更的
...
​	在多SQL工单联合以及部署工单结合这块也是个隐患, 即如果有工单串联关系需要在备注中标明前置工单, 审批工单时需要从最久时间往最近时间开始审, 不能使用就近原则

​	检查SQL成功, 执行直接失败; 执行成功, 实际修改数据不没有效果
